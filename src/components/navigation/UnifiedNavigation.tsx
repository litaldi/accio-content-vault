...
export const UnifiedNavigation: React.FC = () => {
  ...
  const visibleItems = navigationItems.filter(item => {
    ...
    const publicPages = ['/', '/features', '/playground', '/contact'];
    // Protected pages only for authenticated users
    const protectedPages = ['/dashboard', '/search', '/collections', '/analytics', '/integrations', '/account'];
    
    if (publicPages.includes(item.href)) return true;
    if (protectedPages.includes(item.href)) return !!user;
    return false;
  });
            ...
            <div className="pt-3 border-t border-border mt-3 space-y-2">
              {user ? (
                <Button variant="outline" className="w-full" asChild>
                  <Link to="/account" onClick={closeMobileMenu}>Account Settings</Link>
                </Button>
              ) : (
                <>
                  <Button variant="outline" className="w-full" asChild>
