
export const copy = {
  headlines: {
    hero: "Transform Information Into Personal Intelligence",
    subhero: "Stop losing valuable insights in digital chaos. Accio captures, organizes, and transforms any content into searchable knowledge that grows smarter with every addition."
  },
  buttons: {
    getStarted: "Start Building for Free",
    seeHowItWorks: "See How It Works",
    signUp: "Start Building for Free",
    contact: "Get in Touch"
  },
  trust: {
    noCredit: "No credit card required",
    freeTrial: "14-day free trial",
    setupTime: "Setup in 30 seconds"
  },
  marketing: {
    social: {
      facebook: "https://facebook.com/accio",
      instagram: "https://instagram.com/accio",
      discord: "https://discord.gg/accio",
      twitter: "https://twitter.com/accio"
    }
  },
  onboarding: {
    welcome: "Welcome to Accio",
    getStarted: "Let's get you started with your knowledge management journey"
  },
  emptyStates: {
    noContent: {
      title: "No content yet",
      description: "Start building your knowledge base by adding your first item",
      action: "Add Content"
    },
    noResults: {
      title: "No results found",
      description: "Try adjusting your search terms or filters",
      action: "Clear Filters"
    },
    noCollections: {
      title: "No collections yet",
      description: "Create your first collection to organize your content",
      action: "Create Collection"
    },
    loading: {
      title: "Loading...",
      description: "Please wait while we fetch your content",
      action: ""
    }
  },
  accessibility: {
    toggleTheme: "Toggle theme"
  },
  microcopy: {
    settings: {
      profile: "Profile Settings",
      preferences: "Preferences",
      security: "Security"
    }
  }
};
